      ...
      <div className={reduced ? "py-16" : "sticky top-0 flex min-h-screen flex-col justify-center py-16"}>
        <div className="container mx-auto px-6">
          <div className="mb-8 max-w-3xl">
            <p className="label-eyebrow mb-4">{reduced ? "Scenes 02–04" : copy.eyebrow}</p>
          ...
          <div className="mb-8 border-y border-rule py-4">
            <p className="label-eyebrow mb-3">Choose an input</p>
                  ...
                  <button
                    ...
                    className={[
                      ...
                      active
                        ...
                        : "border-rule text-muted-foreground hover:border-primary/60",
                    ].join(" ")}
                  >
                    {input.label}
                  </button>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_1fr] lg:items-start">
            ...
            <div className="relative h-[22rem] sm:h-[26rem]">
              {launchInputs.map((input, idx) => {
                ...
                return (
                  <article
                    key={input.id}
                    className="stage-layer absolute w-[88%] border bg-card p-3.5 shadow-card sm:w-[46%]"
                    style={{
                      ...style,
                      borderColor: isSelected && !scattered ? "hsl(var(--primary))" : "hsl(var(--rule))",
                    }}
                  >
            ...
            <div className="space-y-6">
              <div
                className="stage-layer border bg-card p-6 shadow-card"
                style={{
                  borderColor: stage === 1 ? "hsl(var(--primary))" : "hsl(var(--rule))",
                  opacity: stage === 0 && !reduced ? 0.35 : 1,
                }}
              >
                <p className="label-eyebrow mb-3">The instruction</p>
                <p className="font-mono-prompt text-sm leading-relaxed text-foreground">
              ...
              <div
                className="stage-layer border bg-card p-6 shadow-card"
                style={{
                  borderColor: stage === 2 ? "hsl(var(--accent))" : "hsl(var(--rule))",
              ...
              >
                <div className="mb-4 flex items-baseline justify-between gap-4 border-b border-rule pb-3">
                  <div>
                    <p className="label-eyebrow">{selected.output.kind}</p>
                    <h3 className="font-display text-xl">{selected.output.title}</h3>
                  </div>
                </div>
                <dl className="space-y-3">
                  {selected.output.lines.map((line) => (
                    <div key={line.label} className="grid gap-1 sm:grid-cols-[9.5rem_1fr] sm:gap-4">
                      <dt className="label-eyebrow">{line.label}</dt>
                      <dd className="text-sm leading-relaxed text-foreground">{line.value}</dd>
                    </div>
                  ))}
                </dl>
                <Link
                  to={`/workflow/${selected.output.workflowId}`}
                  className="mt-5 inline-flex items-center gap-2 border-t border-rule pt-4 text-sm font-medium text-primary"
                >
